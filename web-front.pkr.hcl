...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      # add -p so it creates the parent directory
      "sudo mkdir -p /web/html",
      "sudo chown ubuntu:ubuntu /web/html"
    ]
  }
  ...
  provisioner "file" {
    ...
    destination = "/tmp/web/nginx.conf" # according to the nginx script, the nginx.conf is in the /tmp/web
  }
